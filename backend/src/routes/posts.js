...
// ✅ Get all posts (feed) with proper media URL and fields
router.get('/', requireAuth, async (req, res) => {
  try {
    const { kind } = req.query;
    const filter = {};

    if (kind && ['text', 'image', 'voice'].includes(kind)) {
      filter.kind = kind;
    }

    // Select only useful fields including mediaUrl
    const posts = await Post.find(filter)
      .sort({ createdAt: -1 })
      .limit(200)
      .select('author kind text mediaUrl likes comments sentiment createdAt')
      .populate('author', 'username') // optional, for display
      .lean();

    // Add full URL to media (so frontend gets complete path)
    const baseUrl = process.env.BASE_URL || 'http://localhost:5000';
    const updatedPosts = posts.map(p => ({
      ...p,
      mediaUrl: p.mediaUrl ? `${baseUrl}${p.mediaUrl}` : null,
    }));
